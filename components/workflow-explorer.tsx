...
export function WorkflowExplorer({ skill }: { skill: Skill }) {
  ...
  return <div className="detail-app">
    ...
    <main className="detail-main">
      <section className="detail-intro"><div><p className="eyebrow">{inspected?"Workflow inspection":"Skill blueprint"}</p><h1>{skill.name}</h1><span className="category-chip detail-category" style={{"--category":skill.color} as React.CSSProperties}><i/>{categoryById[skill.category].label}</span></div><p>{skill.purpose}</p><div className={`intro-rank ${inspected?"":"pending"}`}><span>Power</span><strong>{inspected?skill.power.toLocaleString():"—"}</strong><small>{skill.scout}</small></div></section>
      <div className="detail-grid">
        ...
        <aside className={`skill-inspector ${sheet?"open":""}`}><button className="sheet-handle" onClick={()=>setSheet(!sheet)}><span/><small>Selected step</small><b>{node.label}</b><i>{sheet?"×":"↑"}</i></button>
          <section className="inspector-block scout-block"><div className="block-heading"><p className="eyebrow">{inspected?"Scout reading":"Blueprint status"}</p><span>{skill.scout}</span></div><div className="scout-line"><div><small>Power level</small><strong style={{color:skill.color}}>{inspected?skill.power.toLocaleString():"—"}</strong></div><dl><dt>Footprint</dt><dd>{skill.footprint}</dd><dt>Recorded installs</dt><dd>{skill.installs.toLocaleString()}</dd></dl></div>{inspected?<div className="quality-list">{skill.qualities.map(q=><div key={q.label}><span>{q.label}</span><i>{Array.from({length:10},(_,i)=><b key={i} className={i<q.score?"on":""} style={{"--skill":skill.color} as React.CSSProperties}/>)}</i><em>{q.word}</em></div>)}</div>:<p className="blueprint-note">This first-pass workflow is derived from catalog metadata. A source inspection will replace it with evidence-backed steps and a Power assessment.</p>}<p className="method-note">Power is our assessment. Rank is ecosystem behavior.</p></section>
