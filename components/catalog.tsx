...
export function Catalog({ skills }: { skills: Skill[] }) {
  ...
  return <><Header search={search} onSearch={setSearch}/><main>
    <section className="hero shell"><div><p className="eyebrow">The inspected skill catalog</p><h1>Find skills<br/>worth loading.</h1><p className="hero-copy">Popularity shows what the ecosystem installs. Power shows what a skill appears capable of after inspection.</p></div><div className="metric-key"><div><span className="metric-dot observed"/><strong>Popularity is observed</strong><small>Recorded installs, rank, and movement.</small></div><div><span className="metric-dot assessed"/><strong>Power is assessed</strong><small>Guidance, efficiency, coverage, and value.</small></div></div></section>
    <section className="shell featured"><div className="section-heading"><div><p className="eyebrow">Highest power</p><h2>Five exceptional Skillprints</h2></div><p>Static inspection · weekly audit</p></div><div className="power-deck">{featured.map((skill,index)=><Link className="power-card" href={`/skill/${skill.slug}`} key={skill.slug} style={{"--skill":skill.color,"--power":`${skill.power/100}%`} as React.CSSProperties}><div className="card-meta"><span>Power {index+1}</span><b>{skill.power>9000 ? "Over 9000" : skill.scout}</b></div><h3>{skill.name}</h3><p>{skill.purpose}</p><div className="card-power"><strong>{skill.power.toLocaleString()}</strong><span>/ 10,000</span></div><div className="power-track"><i/></div></Link>)}</div></section>
