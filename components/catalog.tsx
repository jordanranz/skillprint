...
function CategoryChip({ skill }: { skill: Skill }) {
...
export function Catalog({ skills }: { skills: Skill[] }) {
  ...
  const categorySkills = useMemo(() => category === "all" ? skills : skills.filter(skill => skill.category === category), [skills,category]);
  ...
  const filtered = useMemo(() => [...categorySkills]
    ...
    .sort((a,b) => sort === "power" ? b.power-a.power : sort === "trending" ? b.trend-a.trend : sort === "updated" ? a.updatedDays-b.updatedDays : b.installs-a.installs), [categorySkills,tags,search,sort]);
  ...
  return <><Header search={search} onSearch={setSearch}/><main>
    ...
    <section className="shell ranking"><div className="section-heading rank-heading"><div><p className="eyebrow">Top 100 preview</p><h2>{labels[sort]}</h2></div><div className="sort-control" role="group" aria-label="Sort skills">{(["popular","power","trending","updated"] as Sort[]).map(value=><button key={value} className={sort===value?"active":""} onClick={()=>setSort(value)}>{value === "popular" ? "Popular" : value[0].toUpperCase()+value.slice(1)}</button>)}</div></div>
      ...
      <div className="skill-ledger"><div className="ledger-head"><span>Rank</span><span>Skill</span><span>Purpose</span><span>Power</span><span>Installs</span><span/></div>{filtered.length ? filtered.map((skill,index)=><Link className="skill-row" href={`/skill/${skill.slug}`} key={skill.slug}><span className="rank-number">{String(index+1).padStart(2,"0")}</span><span className="ledger-name"><b>{skill.name}</b><span className="ledger-sub"><small>{skill.owner}</small><CategoryChip skill={skill}/></span></span><span className="ledger-purpose">{skill.purpose}</span><span className="ledger-power"><b>{skill.power.toLocaleString()}</b><small>{skill.scout}</small></span><span className="ledger-installs"><b>{formatInstalls(skill.installs)}</b><small className={skill.trend>=0?"up":"down"}>{skill.trend>=0?"↑":"↓"}{Math.abs(skill.trend)}</small></span><ArrowIcon className="row-arrow"/></Link>) : <div className="empty-state"><strong>No matching skills</strong><span>Clear a filter or try another search.</span></div>}</div>
