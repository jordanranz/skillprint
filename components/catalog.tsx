...
const labels: Record<Sort,string> = { popular: "Ranked by recorded installs", power: "Ranked by assessed Power" };
...
export function Catalog({ skills, ranking, generatedAt }: { skills: Skill[]; ranking: CatalogEntry[]; generatedAt: string }) {
  ...
  const filtered = useMemo(() => [...categorySkills]
    ...
    .sort((a,b) => sort === "power" ? (b.power ?? -1)-(a.power ?? -1) || a.rank-b.rank : a.rank-b.rank), [categorySkills,tags,search,sort]);
  ...
  return <><Header search={search} onSearch={setSearch}/><main>
    ...
    <section className="shell featured"><div className="section-heading"><div><p className="eyebrow">Highest power</p><h2>Five exceptional Skillprints</h2></div><div className="scoring-context"><span>Power updates when source changes</span><details><summary>How scoring works</summary><p>Power evaluates guidance, efficiency, coverage, and practical value. Popularity is recorded installs and does not increase a skill’s Power score.</p></details></div></div><div className="power-deck">{featured.map((skill,index)=><Link className="power-card" href={`/skill/${skill.slug}`} key={skill.slug} style={{"--skill":skill.color,"--power":`${skill.power/100}%`} as React.CSSProperties}><div className="card-meta"><span>Power {index+1}</span><span className="card-tags">{skill.tags.slice(0,2).map(tag=><b key={tag}>{tag}</b>)}</span></div><CategoryChip skill={skill}/><h3>{skill.name}</h3><p>{skill.purpose}</p><div className="card-power"><strong>{skill.power.toLocaleString()}</strong><span>/ 10,000</span></div><div className="power-track" aria-label={`${skill.power.toLocaleString()} out of 10,000 Power`}><i/></div></Link>)}</div><p className="deck-hint" aria-hidden="true">Swipe to compare <span>→</span></p></section>
    <section className="shell ranking"><div className="section-heading rank-heading"><div><p className="eyebrow">Current top 100</p><h2>{labels[sort]}</h2></div><div className="sort-control" role="group" aria-label="Sort skills">{(["popular","power"] as Sort[]).map(value=><button key={value} className={sort===value?"active":""} onClick={()=>setSort(value)}>{value === "popular" ? "Popular" : "Power"}</button>)}</div></div>
      ...
      <div className={`skill-ledger sort-${sort}`}><div className="ledger-head"><span>All-time rank</span><span>Skill</span><span>Category</span><span>Purpose</span><span>Recorded installs</span><span>Power</span><span/></div>{filtered.length ? filtered.map((skill)=><Link className="skill-row" href={`/skill/${skill.detailSlug}`} key={skill.id} style={{"--category":skill.color} as React.CSSProperties}><span className="rank-number">{String(skill.rank).padStart(2,"0")}</span><span className="ledger-name"><b>{skill.name}</b><small>{skill.owner}</small></span><CategoryChip skill={skill}/><span className="ledger-purpose">{skill.purpose}</span><span className="ledger-installs"><b>{formatInstalls(skill.installs)}</b></span><span className={`ledger-power ${skill.power===null?"pending":""}`}><b>{skill.power?.toLocaleString() ?? "—"}</b><small>{skill.scout}</small></span><ArrowIcon className="row-arrow"/></Link>) : <div className="empty-state"><strong>No matching skills</strong><span>Clear a filter or try another search.</span></div>}</div>
